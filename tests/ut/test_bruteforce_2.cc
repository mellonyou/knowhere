...
#define MAX_LOOP 2
TEST_CASE("Test Brute Force", "[float vector bf16]") {
    using Catch::Approx;
    ...
    const auto train_ds = GenBFloat16DataSet(nb, dim);
    const auto query_ds = CopyBFloat16DataSet(train_ds, nq);
	 ...
	 for (const float percentage : bitset_percentages) {
		 ...
		 for (const auto& gen_func : gen_bitset_funcs) {
			 ...
			 for (int i = 0; i < MAX_LOOP; i++)
			 {
				 ...
				 auto res = knowhere::BruteForce::Search<knowhere::bf16>(train_ds, query_ds, conf, bitset);
